...
fn vec_as_u8_32_array(vec: &Vec<u8>) -> [u8; 98] {
    assert!(vec.len() <= 98);
    let mut array = [0u8; 98];
...
pub fn prepare_setup<T: Field>(
    ...
    // We're building { i32, i32, i8[98] }
    const STRUCT_SIZE: usize = 106;
...
pub fn prepare_generate_proof<T: Field>(
    ...
    pk_path: &str,
    proof_path: &str,
) -> (CString, CString, Vec<[u8; 98]>, usize, Vec<[u8; 98]>, usize) {
    let pk_path_cstring = CString::new(pk_path).unwrap();
    ...
    let mut public_inputs_arr: Vec<[u8; 98]> = vec![[0u8; 98]; public_inputs_length];
    // length must not be zero here, so we apply the max function
    let mut private_inputs_arr: Vec<[u8; 98]> = vec![[0u8; 98]; max(private_inputs_length, 1)];
