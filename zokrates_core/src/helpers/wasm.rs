...
mod tests {
    ...
    fn check_invalid_field_size() {
        ...
        let input = vec![BN128::from(65536)];
        let outputs = id.execute(&input);
        assert_eq!(
            outputs,
            Err(String::from(
    ...
    fn check_identity_3_bytes() {
        let id = WasmHelper::from_hex(WasmHelper::IDENTITY_WASM);
        let input = vec![BN128::from(16777216)];
        let outputs = id.execute(&input).expect("Identity call failed");
        assert_eq!(outputs, input);
    }
    ...
    fn check_bits() {
        ...
        let input = vec![BN128::from(0xdeadbeef as u32)];
        ...
        for i in 0..32 {
            ...
            assert_eq!(outputs[(253 - i) as usize], BN128::from(bitval as i32));
        }

        for i in 32..254 {
            assert_eq!(outputs[(253 - i) as usize], BN128::from(0));
        }
    }
