...
mod tests {
    ...
    fn zero() {
        // 0
        let zero = FlatExpression::Number(BN128::from(0));
        let expected: LinComb<BN128> = LinComb::zero();
    ...
    fn one() {
        // 1
        let one = FlatExpression::Number(BN128::from(1));
        let expected: LinComb<BN128> = FlatVariable::one().into();
    ...
    fn forty_two() {
        // 42
        let one = FlatExpression::Number(BN128::from(42));
        let expected: LinComb<BN128> = LinComb::summand(42, FlatVariable::one());
        assert_eq!(LinComb::from(one), expected);
    }
    ...
    fn linear_combination() {
        ...
        let add = FlatExpression::Add(
            box FlatExpression::Mult(
                box FlatExpression::Number(BN128::from(42)),
                box FlatExpression::Identifier(FlatVariable::new(42)),
            ),
            box FlatExpression::Mult(
                box FlatExpression::Number(BN128::from(21)),
                box FlatExpression::Identifier(FlatVariable::new(21)),
            ),
        );
        let expected: LinComb<BN128> = LinComb::summand(42, FlatVariable::new(42))
