...
void serializeVerificationKeyToFile(r1cs_se_ppzksnark_verification_key<ppT> vk, const char* vk_path){
  ...
  ss << "\t\tvk.H = " << outputPointG2AffineAsHex<n, G2T>(vk.H) << endl;
  ss << "\t\tvk.Galpha = " << outputPointG1AffineAsHex<n, G1T>(vk.G_alpha) << endl;
  ss << "\t\tvk.Hbeta = " << outputPointG2AffineAsHex<n, G2T>(vk.H_beta) << endl;
  ss << "\t\tvk.Ggamma = " << outputPointG1AffineAsHex<n, G1T>(vk.G_gamma) << endl;
  ss << "\t\tvk.Hgamma = " << outputPointG2AffineAsHex<n, G2T>(vk.H_gamma) << endl;
  ss << "\t\tvk.query.len() = " << queryLength << endl;
  for (mp_size_t i = 0; i < queryLength; ++i)
  {
      auto vkqueryi = outputPointG1AffineAsHex<n, G1T>(vk.query[i]);
...
template<mp_size_t n, typename ppT, typename G1T, typename G2T>
void exportProof(r1cs_se_ppzksnark_proof<ppT> proof, const char* proof_path, const uint8_t* public_inputs,
    ...
    ss << "{" << "\n";
      ss << "\t\"proof\":" << "\n";
        ss << "\t{" << "\n";
          ss << "\t\t\"A\":" <<outputPointG1AffineAsHexJson<n, G1T>(proof.A) << ",\n";
          ss << "\t\t\"B\":" << "\n";
            ss << "\t\t\t" << outputPointG2AffineAsHexJson<n, G2T>(proof.B) << ",\n";
          ss << "\t\t\n";
          ss << "\t\t\"C\":" <<outputPointG1AffineAsHexJson<n, G1T>(proof.C) << ",\n";
        ss << "\t}," << "\n";
      //add input to json
      ss << "\t\"input\":" << "[";
...
{
  auto cs = gm17::createConstraintSystem<n, ppT>(A, B, C, A_len, B_len, C_len, constraints, variables, inputs);
  ...
  gm17::serializeVerificationKeyToFile<n, ppT, G1T, G2T>(keypair.vk, vk_path);
  return true;
}
...
bool generate_proof(const char* pk_path, const char* proof_path, const uint8_t* public_inputs, int public_inputs_length, const uint8_t* private_inputs, int private_inputs_length)
{
  ...
  gm17::exportProof<n, ppT, G1T, G2T>(proof, proof_path, public_inputs, public_inputs_length);
  return true;
}
...
bool _gm17_setup(const uint8_t* A, const uint8_t* B, const uint8_t* C, int A_len, int B_len, int C_len, int constraints, int variables, int inputs, const char* pk_path, const char* vk_path)
{
  ...
  return gm17::setup<libff::alt_bn128_r_limbs, libff::alt_bn128_pp, libff::alt_bn128_G1, libff::alt_bn128_G2>(A, B, C, A_len, B_len, C_len, constraints, variables, inputs, pk_path, vk_path);
}
...
bool _gm17_generate_proof(const char* pk_path, const char* proof_path, const uint8_t* public_inputs, int public_inputs_length, const uint8_t* private_inputs, int private_inputs_length)
{
  ...
  libff::inhibit_profiling_counters = true;
  libff::alt_bn128_pp::init_public_params();
  return gm17::generate_proof<libff::alt_bn128_r_limbs, libff::alt_bn128_pp, libff::alt_bn128_G1, libff::alt_bn128_G2>(pk_path, proof_path, public_inputs, public_inputs_length, private_inputs, private_inputs_length);
}
